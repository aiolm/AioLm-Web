...
`@aiolm/benchmark-contracts` 0.5.1 owns the shared data contract: DTOs,
...
```json
"@aiolm/benchmark-contracts": "file:vendor/aiolm-benchmark-contracts-0.5.1.tgz"
```
...
```sh
CONTRACTS_TARBALL_PATH=./aiolm-benchmark-contracts-0.5.1.tgz npm run contracts:check
```
