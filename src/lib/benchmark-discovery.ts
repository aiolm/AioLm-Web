...
export const TEXT_FILTER_KEYS = ["q", "model", "hardware", "vendor", "gpu", "cpu", "os", "arch", "runtime", "backend", "mode", "method", "workload", "flash_attention", "cache_type_k", "cache_type_v", "split_mode"] as const;
...
export function textValues(summary: BenchmarkSummary, field: TextFilterKey): string[] {
  if (field === "q") return [...new Set(TEXT_FILTER_KEYS.filter(k => k !== "q").flatMap(k => textValues(summary, k)).concat(summary.setup?.runtime_version ?? [], summary.status))];
...
export function numericValue(summary: BenchmarkSummary, field: RangeFilterKey): number | null {
  return summary.setup?.[field === "context" ? "context_size" : field === "vram" ? "vram_mb" : field] ?? null;
}
...
export function sortValue(summary: BenchmarkSummary, sort: BenchmarkSort): number | null {
  if (sort.startsWith("context_")) return summary.setup?.context_size ?? null;
